...
func (es *EchoServer) registerRoutes() {
	...
	authed.GET("/accounts/unlinked/github/:org", es.h.HandleUnmatchedGitHub)
	authed.GET("/accounts/unlinked/entra", es.h.HandleUnmatchedEntra)
	authed.GET("/accounts/unlinked/google-workspace", es.h.HandleUnmatchedGoogleWorkspace)
	authed.GET("/accounts/unlinked/aws", es.h.HandleUnmatchedAWS)
	authed.GET("/accounts/unlinked/datadog/:site", es.h.HandleUnmatchedDatadog)
	...
	admin.POST("/app-assets/:id/review", es.h.HandleAppAssetReviewUpdate)
	admin.POST("/app-assets/:id/grants/:credentialID/revoke", es.h.HandleAppAssetGrantRevoke)
	admin.POST("/oauth-apps/:id/review", es.h.HandleConnectedAppReviewUpdate)
