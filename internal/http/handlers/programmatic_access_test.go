...
func TestCredentialExpiresInLabelAt(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name:  "future less than day",
			value: timestamptz(now.Add(23 * time.Hour)),
			want:  "in <1d",
		},
		{
			name:  "future multiple days",
			value: timestamptz(now.Add(49 * time.Hour)),
			want:  "in 2d",
		},
		{
			name:  "expired less than day",
			value: timestamptz(now.Add(-6 * time.Hour)),
			want:  "expired <1d ago",
		},
		{
			name:  "expired multiple days",
			value: timestamptz(now.Add(-50 * time.Hour)),
			want:  "expired 2d ago",
		},
		{
			name:  "invalid",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := credentialExpiresInLabelAt(tc.value, now); got != tc.want {
				t.Fatalf("credentialExpiresInLabelAt() = %q, want %q", got, tc.want)
			}
		})
	}
}

func TestCredentialLastUsedAgoLabelAt(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{
			name:  "future skew",
			value: timestamptz(now.Add(1 * time.Hour)),
			want:  "just now",
		},
		{
			name:  "recent less than day",
			value: timestamptz(now.Add(-8 * time.Hour)),
			want:  "<1d ago",
		},
		{
			name:  "older multiple days",
			value: timestamptz(now.Add(-73 * time.Hour)),
			want:  "3d ago",
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := credentialLastUsedAgoLabelAt(tc.value, now); got != tc.want {
				t.Fatalf("credentialLastUsedAgoLabelAt() = %q, want %q", got, tc.want)
			}
		})
	}
}
