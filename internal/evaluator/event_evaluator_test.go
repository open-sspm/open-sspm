...
func TestEvaluateEventProducesEventSignal(t *testing.T) {
	t.Parallel()
	...
	if result.PolicyPackID != "opensspm.event.v1" || result.PolicyPackVersion == "" {
		t.Fatalf("policy pack = %q/%q, want canonical event pack", result.PolicyPackID, result.PolicyPackVersion)
	}
}
