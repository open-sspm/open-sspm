...
const listDashboardPrivilegedAccessBuckets = `-- name: ListDashboardPrivilegedAccessBuckets :many
...
ranked AS (
  SELECT
    source_kind, source_name, bucket_key, bucket_label, severity, affected_count, entitlement_count,
    row_number() OVER (
...
func (q *Queries) ListDashboardPrivilegedAccessBuckets(ctx context.Context, arg ListDashboardPrivilegedAccessBucketsParams) ([]ListDashboardPrivilegedAccessBucketsRow, error) {
	rows, err := q.db.Query(ctx, listDashboardPrivilegedAccessBuckets, arg.BucketLimit, arg.ConfiguredSourceKinds, arg.ConfiguredSourceNames)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListDashboardPrivilegedAccessBucketsRow
	for rows.Next() {
		var i ListDashboardPrivilegedAccessBucketsRow
		if err := rows.Scan(
			&i.SourceKind,
			&i.SourceName,
			&i.BucketKey,
...
const listDashboardSourceAccountSummaries = `-- name: ListDashboardSourceAccountSummaries :many
WITH configured_sources AS (
  SELECT
    k.kind AS source_kind,
    n.name AS source_name,
...
func (q *Queries) ListDashboardSourceAccountSummaries(ctx context.Context, arg ListDashboardSourceAccountSummariesParams) ([]ListDashboardSourceAccountSummariesRow, error) {
	rows, err := q.db.Query(ctx, listDashboardSourceAccountSummaries, arg.ConfiguredSourceKinds, arg.ConfiguredSourceNames)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListDashboardSourceAccountSummariesRow
	for rows.Next() {
		var i ListDashboardSourceAccountSummariesRow
		if err := rows.Scan(
			&i.SourceKind,
			&i.SourceName,
			&i.IdentityCount,
			&i.AccountCount,
			&i.ManagedAccountCount,
			&i.UnmanagedAccountCount,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
