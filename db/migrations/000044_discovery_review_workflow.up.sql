ALTER TABLE governance_subject_overrides
  ADD COLUMN IF NOT EXISTS review_disposition TEXT NOT NULL DEFAULT 'unreviewed',
  ADD COLUMN IF NOT EXISTS review_owner_identity_id BIGINT REFERENCES identities(id) ON DELETE SET NULL,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'governance_subject_overrides_review_disposition_check'
  ) THEN
    ALTER TABLE governance_subject_overrides
      ADD CONSTRAINT governance_subject_overrides_review_disposition_check CHECK (
        review_disposition IN ('unreviewed', 'under_review', 'sanctioned', 'tolerated', 'replace')
      );
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_governance_subject_overrides_kind_review_disposition
  ON governance_subject_overrides (subject_kind, review_disposition);

CREATE INDEX IF NOT EXISTS idx_governance_subject_overrides_review_owner_identity_id
  ON governance_subject_overrides (review_owner_identity_id);

CREATE INDEX IF NOT EXISTS idx_governance_subject_overrides_replacement_saas_app_id
  ON governance_subject_overrides (replacement_saas_app_id);

CREATE TABLE IF NOT EXISTS saas_app_review_decisions (
  ...
  review_owner_identity_id BIGINT REFERENCES identities(id) ON DELETE SET NULL,
  review_disposition TEXT NOT NULL DEFAULT 'unreviewed',
  ticket_ref TEXT NOT NULL DEFAULT '',
  notes TEXT NOT NULL DEFAULT '',
  follow_up_due_date DATE,
  replacement_saas_app_id BIGINT REFERENCES saas_apps(id) ON DELETE SET NULL,
  changed_by_auth_user_id BIGINT REFERENCES auth_users(id) ON DELETE SET NULL,
  changed_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT saas_app_review_decisions_disposition_check CHECK (
    review_disposition IN ('unreviewed', 'under_review', 'sanctioned', 'tolerated', 'replace')
  )
);

CREATE INDEX IF NOT EXISTS idx_saas_app_review_decisions_saas_app_id_changed_at
  ON saas_app_review_decisions (saas_app_id, changed_at DESC);
